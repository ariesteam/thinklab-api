 ...
 * 
 * Observations returned by get() can be used to initialize a contextualizer
 * along with the context that produced the list.
 * 
 * @author Ferdinando
 * 
 */
public interface IObservationList extends Iterable<IObservation> {
	...
	/**
	 * Return the n-th observation (please be lazy). The returned observation
	 * links anything that needs to be computed based on what's already
	 * in the context. It should only return null if the context that has 
	 * generated this already contains states for all the observables in the 
	 * original model. 
	 * 
	 * @param index
	 * @return
	 */
	public abstract IObservation get(int index) throws ThinklabException;
